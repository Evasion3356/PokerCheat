...
struct ColorRgba
{
	// unsigned char, not byte: byte is ambiguous under recent C++ standards
	// where both <rpcndr.h>'s global `byte` and std::byte are visible
	// (scriptmenu.h has `using namespace std;` below, matching the original
	// 2019 SDK sample).
...
public:
	...
	{
		// Stock natives.h declares these char* (not const char*) -- invoke<>
